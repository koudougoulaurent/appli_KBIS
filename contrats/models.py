...
class Quittance(models.Model):
    ...
    def _generer_recu_kbis_dynamique(self, user=None):
        """Génère un récépissé KBIS dynamique pour la quittance avec le format correct."""
        import sys
        import os
        from datetime import datetime
        from decimal import Decimal
        ...
        try:
            ...
            try:
                montant_loyer = float(Decimal(self.montant_loyer)) if self.montant_loyer else 0.0
                montant_charges = float(Decimal(self.montant_charges)) if self.montant_charges else 0.0
                montant_total = montant_loyer + montant_charges
            except (ValueError, TypeError):
                montant_total = float(Decimal(self.montant_total)) if self.montant_total else 0.0
            
            # Formater le mois réglé
            mois_francais = [
                'janvier', 'février', 'mars', 'avril', 'mai', 'juin',
                'juillet', 'août', 'septembre', 'octobre', 'novembre', 'décembre'
            ]
            mois_regle = f"{mois_francais[self.mois.month - 1]} {self.mois.year}"
            
            # Données du récépissé
            donnees_recu = {
                'numero': numero_recu,
                'date': self.date_emission.strftime('%d-%b-%y') if self.date_emission else datetime.now().strftime('%d-%b-%y'),
                'code_location': code_location,
                'recu_de': recu_de,
                'montant': montant_total,
                'mois_regle': mois_regle,
                'type_paiement': 'Loyer',
                'mode_paiement': 'Espèces',  # Par défaut, peut être ajusté selon vos besoins
                'quartier': quartier,
                'loyer_mensuel': montant_loyer,
                'charges_mensuelles': montant_charges,
                'total_mensuel': montant_total,
            }
            
            # Générer le document unifié (type récépissé de loyer)
            return DocumentKBISUnifie.generer_document_unifie(donnees_recu, 'recu_loyer', user=user)
            
        except Exception as e:
            print(f"Erreur génération récépissé quittance KBIS: {e}")
            import traceback
            traceback.print_exc()
            return None
