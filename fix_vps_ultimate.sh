...
# Script ultime pour corriger tous les problemes sur le VPS
echo "Correction ultime des problemes sur le VPS..."
...
cat > fix_models_ultimate.py << 'EOF'
#!/usr/bin/env python3
import re
...
def fix_models():
    ...
    with open('paiements/models.py.backup_ultimate', 'w', encoding='utf-8') as f:
        f.write(content)
    ...
    # Corriger la relation OneToOneField en ForeignKey avec null=True, blank=True
...
python fix_models_ultimate.py

# Supprimer le script temporaire
rm fix_models_ultimate.py

# 4. Corriger les erreurs dans la vue
echo "4. Correction des erreurs dans la vue..."

if [ -f "paiements/views_recus.py" ]; then
    echo "Correction des erreurs dans views_recus.py..."
    
    # Corriger les references aux champs qui n'existent pas
    sed -i 's/recu\.montant/recu.recapitulatif.total_net_a_payer/g' "paiements/views_recus.py"
    sed -i 's/recu\.get_type_display()/recu.get_type_recu_display()/g' "paiements/views_recus.py"
    sed -i 's/recu\.recapitulatif\.calculer_totaux_bailleur()/recu.recapitulatif/g' "paiements/views_recus.py"
    
    echo "  OK: Erreurs dans views_recus.py corrigees"
fi
...
echo "Correction ultime terminee avec succes!"
