/**
 * Système de Notifications Dynamiques Ultra-Fonctionnel
 * Gestion complète des notifications en temps réel avec interface avancée
 */

class NotificationManager {
    constructor() {
        this.pollInterval = 10000; // 10 secondes
        this.retryInterval = 5000; // 5 secondes
        this.maxRetries = 3;
        this.retryCount = 0;
    ...
    init() {
        console.log('🔔 Initialisation du système de notifications dynamiques');
    ...
    async loadUserPreferences() {
        try {
            ...
            if (data.success) {
                this.soundEnabled = data.preferences.browser_notifications;
                this.toastEnabled = data.preferences.browser_notifications;
            }
        } catch (error) {
    ...
    async loadInitialNotifications() {
        try {
            const response = await fetch('/notifications/api/recentes/?limit=10');
            const data = await response.json();
            ...
            <div class="d-flex align-items-start">
                ...
                <div class="notification-actions">
                    ${!notification.is_read ? 
                        `<button class="btn btn-sm btn-outline-primary" onclick="notificationManager.markAsRead(${notification.id})" title="Marquer comme lu">
                            <i class="bi bi-check"></i>
                        </button>` : ''
                    }
                    <button class="btn btn-sm btn-outline-danger" onclick="notificationManager.deleteNotification(${notification.id})" title="Supprimer">
    ...
    getNotificationConfig(type) {
        const configs = {
            ...
            'synchronization_complete': { icon: 'bi-check-circle', color: 'success' },
    ...
    initSounds() {
        // Créer les sons de notification
        this.sounds = {
...
window.refreshNotifications = () => notificationManager.refresh();
window.toggleNotificationSound = () => notificationManager.toggleSound();
window.toggleNotificationToast = () => notificationManager.toggleToast();
