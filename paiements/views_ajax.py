...
def get_plan_details(request, plan_id):
    """Récupérer les détails d'un plan pour AJAX - DÉSACTIVÉ"""
    try:
        # plan = get_object_or_404(PlanPaiementPartiel, id=plan_id, is_deleted=False)
        return JsonResponse({'error': 'Fonctionnalité temporairement désactivée'}, status=400)
...
def create_quick_payment(request):
    """Créer un paiement rapide via AJAX - DÉSACTIVÉ"""
    try:
        return JsonResponse({'success': False, 'error': 'Fonctionnalité temporairement désactivée'})
...
def bulk_update_plans(request):
    """Mise à jour en lot des plans - DÉSACTIVÉ"""
    try:
        return JsonResponse({'success': False, 'error': 'Fonctionnalité temporairement désactivée'})
...
def get_plans_data(request):
    """Récupérer les données des plans pour les graphiques - DÉSACTIVÉ"""
    try:
        return JsonResponse({'success': False, 'error': 'Fonctionnalité temporairement désactivée'})
...
def search_plans(request):
    """Recherche avancée des plans - DÉSACTIVÉ"""
    try:
        return JsonResponse({'success': False, 'error': 'Fonctionnalité temporairement désactivée'})
